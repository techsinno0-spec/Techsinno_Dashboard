...
{"important":[{"id":"<inbox_id>","subject":"...","from":"...","priority":"urgent|high","reason":"why important, max 8 words","action":"reply|review|call|pay","draft":"1-2 sentence draft response or action step"}],"unanswered":[{"id":"<inbox_id>","subject":"...","from":"...","daysSinceReceived":N,"reminderDays":1}],"opportunities":[{"id":"<inbox_id>","subject":"...","from":"...","description":"business opportunity, max 10 words"}]}
...
- draft: write as Frank from Techsinno, professional but brief.`;
...
ipcMain.handle('gmail-ai-scan', async () => {
  try {
    ...
    const client = new Anthropic({ apiKey });
    const prompt = `Email assistant for Frank Muland, TECHSINNO (Pty) Ltd, South Africa — IT services, automation, IoT.\n\nINBOX:\n${JSON.stringify(inboxList.map(e=>({id:e.id,from:e.from,subject:e.subject,date:e.date,unread:e.unread})))}\n\nSENT:\n${JSON.stringify(sentList.map(e=>({subject:e.subject,to:e.to,date:e.date})))}\n\nToday: ${new Date().toISOString()}\n\nReturn ONLY JSON: {"important":[{"id":"...","subject":"...","from":"...","priority":"urgent|high","reason":"max 8 words","action":"reply|review|call|pay","draft":"1-2 sentence draft"}],"unanswered":[{"id":"...","subject":"...","from":"...","daysSinceReceived":N,"reminderDays":1}],"opportunities":[{"id":"...","subject":"...","from":"...","description":"max 10 words"}]}`;
    const response = await client.messages.create({ model:'claude-haiku-4-5-20251001', max_tokens:1200, messages:[{role:'user',content:prompt}] });
    const text = (response.content[0]?.text||'{}').trim();
    const match = text.match(/\{[\s\S]*\}/);
    const result = match ? JSON.parse(match[0]) : { important:[], unanswered:[], opportunities:[] };
...
ipcMain.handle('ms-ai-scan', async () => {
  try {
    ...
    const client = new Anthropic({ apiKey });
    const prompt = `Email assistant for Frank Muland, TECHSINNO (Pty) Ltd, South Africa.\n\nINBOX:\n${JSON.stringify(inboxList.map(e=>({id:e.id,from:e.from,subject:e.subject,date:e.date,unread:e.unread})))}\n\nSENT:\n${JSON.stringify(sentList)}\n\nToday: ${new Date().toISOString()}\n\nReturn ONLY JSON: {"important":[{"id":"...","subject":"...","from":"...","priority":"urgent|high","reason":"max 8 words","action":"reply|review|call|pay","draft":"1-2 sentence draft"}],"unanswered":[{"id":"...","subject":"...","from":"...","daysSinceReceived":N,"reminderDays":1}],"opportunities":[{"id":"...","subject":"...","from":"...","description":"max 10 words"}]}`;
    const response = await client.messages.create({ model:'claude-haiku-4-5-20251001', max_tokens:1200, messages:[{role:'user',content:prompt}] });
    const text = (response.content[0]?.text||'{}').trim();
    const match = text.match(/\{[\s\S]*\}/);
    const result = match ? JSON.parse(match[0]) : { important:[], unanswered:[], opportunities:[] };
...
[{"topic":"short topic name","content":"full post text with hashtags"}]`}] });
    ...
    try {
      const r = await client.messages.create({ model:'claude-haiku-4-5-20251001', max_tokens:2200, messages:[{role:'user',content:`Write 2 cold outreach emails for Frank Muland, TECHSINNO (Pty) Ltd, targeting Western Cape businesses. Target different sectors each time (manufacturing, food processing, farming/agriculture, cold storage, medical devices).
Each email: 3-4 sentences, addresses a specific pain point (downtime, load-shedding damage, PCB failure, control system failure), soft CTA (free site audit or no-obligation quote), professional but not generic.

For each sector also provide 5 realistic target companies actually located in the Western Cape, South Africa. Use real well-known SA companies where possible (Distell, Woolworths, Tiger Brands, Sea Harvest, RCL Foods, Boland Pac, Agri Voedsel, Cape Bio Pharms, etc.) or realistic examples. Include the job title of the person Frank should contact.

Return ONLY valid JSON array:
[{
  "targetSector": "sector name",
  "subject": "subject line",
  "body": "email body ending with:\\nBest regards,\\nFrank Muland\\nTECHSINNO (Pty) Ltd\\n+27 XX XXX XXXX\\nfrank@techsinno.com\\nwww.techsinno.com\\nKuilsriver, Western Cape",
  "contactTitle": "e.g. Maintenance Manager or Operations Engineer",
  "targets": [
    {"company": "Company Name", "website": "www.example.co.za", "area": "Cape Town / Paarl / Stellenbosch / etc", "contactTitle": "Maintenance Manager"}
  ]
}]`}] });
      const m = (r.content[0]?.text||'[]').trim().match(/\[[\s\S]*\]/);
      (m ? JSON.parse(m[0]) : []).forEach(item => {
        newItems.push({ id:uid(), type:'cold_email', source:'generated', priority:4, flagType:'outreach', title:'Cold email → '+item.targetSector, reason:'Targeting '+item.targetSector+' sector', to:'', subject:item.subject, body:item.body, provider:'zohomail', targetSector:item.targetSector, contactTitle:item.contactTitle||'', targets:item.targets||[], status:'pending', createdAt:Date.now() });
      });
