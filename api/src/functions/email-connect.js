...
const { getEmailConfig, GMAIL_AUTH_URL, GMAIL_SCOPES, MS_AUTH_URL, MS_SCOPES, ZOHO_SCOPES } = require('../../shared/email');
...
app.http('email-connect', {
  ...
  handler: async (request) => {
    ...
    if (provider === 'zoho_mail') {
      ...
      const url = `https://accounts.zoho.com/oauth/v2/auth?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(ZOHO_SCOPES)}&access_type=offline&prompt=consent&state=${state}`;
      return jsonResponse({ url });
    }
